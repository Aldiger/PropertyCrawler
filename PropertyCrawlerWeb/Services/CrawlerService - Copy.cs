...
namespace PropertyCrawlerWeb.Services
{
    ...
    {
        void Execute(List<PostalCode> postalCodes, PropertyType type, Process process);
        ...
        void UpdatePriceUrlCrawler(List<PostalCode> postalCodes, PropertyType type, Process process);
    }
    public class CrawlerService : ICrawlerService
    {
        ...
        public void Execute(List<PostalCode> postalCodes, PropertyType type, Process process)
        {
            if (ProcessType.LastWeek == process.Type)
            {
                UrlCrawler(postalCodes, type, process);
            }
            else if (ProcessType.LastTwoWeeks == process.Type)
            {
                PropertiesCrawler(postalCodes, process, type);
            }
            else if (ProcessType.Update_Price == process.Type)
            {
                UpdatePriceUrlCrawler(postalCodes, type, process);
            }
            else
            {
        ...
        public void UpdatePriceUrlCrawler(List<PostalCode> postalCodes, PropertyType type, Process process)
        {
            ...
            Parallel.ForEach(partitioner, parallelOptions, (listItem, loopState) =>
            {
                ...
                try
                {
                    if (type == PropertyType.All)
                    {
                        ProcessUpdatePriceUrl(portal, listItem, PropertyType.Sell, urlTypes, processPostalCode);
                        ProcessUpdatePriceUrl(portal, listItem, PropertyType.Rent, urlTypes, processPostalCode);
                    }
                    else
                    {
                        ProcessUpdatePriceUrl(portal, listItem, type, urlTypes, processPostalCode);
                    }
